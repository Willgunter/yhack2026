  ...
  try {
    ...
    if (decision.verdict === 'DENY' || decision.verdict === 'WARN') {
        ...
        const payload = {
            ...
            remediationMeta: decision.remediationMeta
        };
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (username === 'intern_seeron' && password) {
    res.status(200).json({ token: 'mock-jwt-token-123', user: { name: 'Seeron', role: 'intern' } });
  } else if (username && password) {
    res.status(200).json({ token: 'mock-jwt-token-123', user: { name: 'Cmdr. Vane', role: 'senior_dev' } });
  } else {
