...
// const axios = require('axios'); // Removed to avoid packaging resolution issues
...
ipcMain.handle('auth-login', async (event, { username, password }) => {
    if (username && password) {
        ...
        if (username.toLowerCase().startsWith('intern_')) {
            role = 'Intern';
            name = username.replace('intern_', '');
        } else if (username.toLowerCase().startsWith('senior_')) {
            role = 'Senior Dev';
            name = username.replace('senior_', '');
        }
...
ipcMain.on('trigger-remediation', () => {
    console.log('👔 [IPC] Remediation triggered by Senior Dev');
    triggerSeniorRemediation(lastViolationMeta || {
        verdict: 'DENY',
        level: 4,
        reasoning: 'Manual remediation initiated by compliance protocol.',
        surface: 'System'
    });
});
...
function triggerSeniorRemediation() {
    ...
    // Call remediation execution backend via fetch (Node built-in)
  ...
  app.whenReady().then(() => {
    ...
    backendProcess.on('message', (msg) => {
        ...
        if (msg.type === 'governance_alert') {
            ...
            console.log(`🛡️ [Main] Received Governance Alert: ${alert.verdict} - ${alert.reasoning}`);
