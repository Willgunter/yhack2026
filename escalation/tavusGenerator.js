...
async function generateTavusVideo(customScript, replicaId = process.env.TAVUS_REPLICA_ID) {
    try {
        ...
        const response = await axios.post('https://tavusapi.com/v2/videos', payload, {
            headers: {
        ...
        console.log(`✅ Tavus Fast-Video Generated: ${response.data.hosted_url}`);
...
async function escalateViaEmail(level, reasoningTrace, tavusUrl) {
    ...
    try {
        ...
        console.log('✅ Escalation Email Sent Successfully:', response.data.id);
    } catch (err) {
